...
function updateQuantity(input, increment) {
    ...
    input.value = isNaN(value) ? 1 : Math.max(1, value + increment);
}
...
function updateCartDisplay() {
    ...
    storedItems.forEach(item => {
        ...
        itemElement.innerHTML = `
            ...
            <button class="delete-btn" onclick="deleteProduct('${item.name}')">X</button>
        `;
    ...
    checkoutBtn.disabled = storedItems.length === 0;
}
...
function searchProducts() {
    ...
    const products = [
        ...
        { name: 'Multiple pattern sticky notes', page: 'page4.html' },
        { name: 'Abstract floral sticky notes', page: 'page4.html' },
        { name: 'Pink flowers sticky notes', page: 'page4.html' },
        { name: 'Large & small flowers sticky notes', page: 'page4.html' }
    ];
...
function displayOrderSummary() {
    ...
    cartItems.forEach(item => {
        ...
        if (uniqueItems.has(itemName)) {
            uniqueItems.get(itemName).quantity += item.quantity;
        } else {
...
function displayItems(items) {
    ...
    document.getElementById('total-price').textContent = `HK$ ${totalPrice.toFixed(2) + 50}`;
}
...
function initializePage5() {
    ...
    fetch('get_cart.php')
        ...
        .then(cart => {
            ...
            document.getElementById('total-items').textContent = totalItems;
            document.getElementById('total-price').textContent = `HK$${(totalPrice + 50).toFixed(2)}`;
        })
